    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <div class="flex items-center gap-3">
                ...
                <span class="bg-[#5C5DCD]/10 text-[#5C5DCD] text-xs font-bold px-2.5 py-1 rounded-full">{{ $orders->count() }} total</span>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-slate-900">{{ $orders->count() }}</h3>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-slate-900">{{ $orders->where('status', 'pending')->count() }}</h3>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-slate-900">{{ $orders->where('status', 'paid')->count() }}</h3>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-slate-500">{{ $order->email ?? $order->phone ?? '-' }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <!-- Tanggal -->
                        <td class="py-4 px-6">
                            <div class="text-sm font-medium text-slate-900">{{ $order->created_at->format('d M Y') }}</div>
                            <div class="text-xs text-slate-500">{{ $order->created_at->format('H:i') }}</div>
                        </td>

                        <!-- Total -->
                        <td class="py-4 px-6">
                            <div class="font-bold text-[#5C5DCD]">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</div>
                        ...
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="/admin/orders/{{ $order->id }}" class="p-2 text-slate-400 hover:text-[#5C5DCD] hover:bg-[#5C5DCD]/10 rounded-lg transition-colors" title="Detail">
                                    <i class="bi bi-eye-fill text-lg"></i>
                                </a>

                                @if(!$order->tracking_number && !in_array($order->status, ['cancelled', 'failed']))
                                <a href="/admin/orders/{{ $order->id }}#resi-form" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Input Resi">
                                    <i class="bi bi-truck text-lg"></i>
                                </a>
                                @endif

                                @if(in_array($order->status, ['cancelled', 'failed']))
                                <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus pesanan ini?')">
                                    @csrf
                                    @method('DELETE')
