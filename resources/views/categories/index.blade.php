    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
        @if($categories->count() > 0)
            ...
            <div class="animate-[fadeInUp_0.5s_cubic-bezier(0.4,0,0.2,1)_forwards] opacity-0" style="animation-delay: {{ $loop->iteration * 0.08 }}s;">
                <a href="{{ route('categories.show', $category->slug) }}" class="block h-full group">
                    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 text-center h-full hover:-translate-y-2 hover:shadow-xl hover:shadow-emerald-500/10 hover:border-emerald-100 transition-all duration-300">
                        ...
                        <h6 class="font-bold text-slate-800 mb-2 group-hover:text-emerald-600 transition-colors">{{ $category->name }}</h6>
